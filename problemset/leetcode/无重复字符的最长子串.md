...
回到我们的问题，我们使用 HashSet 将字符存储在当前窗口 [i, j)（最初 j = i）中。 然后我们向右侧滑动索引 j，如果它不在 HashSet 中，我们会继续滑动 j。直到 s[j] 已经存在于 HashSet 中。此时，我们找到的没有重复字符的最长子字符串将会以索引 i开头。如果我们对所有的 i这样做，就可以得到答案。
```java
public class Solution {
    public int lengthOfLongestSubstring(String s) {
        int n = s.length();
...
public:
    ...
    int lengthOfLongestSubstring(string s) {
        ...
        // 辅助变量
        int len = s.length();
        int chars[128] = {0};// 由于是字符串，直接用数组代替HashMap
        int maxlength = 0;
        // 遍历统计
        for(int i = 0,j=0;j < len;++j) {
            i = max(chars[s[j]],i);// 更新i,如果已经存在s[j]
            maxlength = max(maxlength,j - i + 1);// 更新最长的长度
            chars[s[j]] = j + 1;// 更新字符的最近一次出现的下标
        }
        return maxlength;
    }
};
